...
document.addEventListener("DOMContentLoaded", () => {
  const start = async () => {
    ...
    function addClone(modelIndex, cloneCount) {
      const originalModel = models[modelIndex].scene;
      const clone = originalModel.clone();
      clone.visible = true;
      // Copia propriedades essenciais do userData (deep copy dos vetores)
      clone.userData = {
      ...
      // Event listener para remover o clone por triple-click (tempo ajustado para 250ms)
      clone.addEventListener("pointerup", (e) => {
        e.stopPropagation();
        clone.userData.cloneTapCount = (clone.userData.cloneTapCount || 0) + 1;
        setTimeout(() => {
          if (clone.userData.cloneTapCount >= 3) {
            if (clone.parent) {
              clone.parent.remove(clone);
              console.log("Clone removido por triple click");
            }
          }
          clone.userData.cloneTapCount = 0;
        }, 250);
      });
      // Posiciona o clone com offset fixo (mantendo o mesmo valor de Z)
      clone.position.set(
    ...
    function updatePlatesForTrash() {
      ...
      iconLeft.addEventListener("click", (e) => {
        e.stopPropagation();
        // Para as plaquinhas, usamos um offset aleatório ou fixo
        addClone( plateModelIndices[currentColorIndex].left, 0 );
      });
      ...
      iconRight.addEventListener("click", (e) => {
        e.stopPropagation();
        addClone( plateModelIndices[currentColorIndex].right, 0 );
      });
    ...
    recicleIcon.addEventListener("click", (e) => {
      e.stopPropagation();
      addClone(13, cloneCountRecicle);
      cloneCountRecicle++;
    });
    document.body.appendChild(recicleIcon);
    
    let cloneCountColete = 0;
    ...
    coleteIcon.addEventListener("click", (e) => {
      e.stopPropagation();
      addClone(12, cloneCountColete);
      cloneCountColete++;
    });
    document.body.appendChild(coleteIcon);
    
    let cloneCountLoop2 = 0;
    ...
    loop2Icon.addEventListener("click", (e) => {
      e.stopPropagation();
      addClone(14, cloneCountLoop2);
      cloneCountLoop2++;
    });
    document.body.appendChild(loop2Icon);
    
    let cloneCountLoop3 = 0;
    ...
    function onPointerUp(e) {
      if (!isDragging && currentTapObject) {
        // Se o objeto for um clone, verifica triple-click para removê-lo
        if (currentTapObject.userData.isClone) {
          currentTapObject.userData.cloneTapCount = (currentTapObject.userData.cloneTapCount || 0) + 1;
          setTimeout(() => {
            if (currentTapObject.userData.cloneTapCount >= 3) {
              if (currentTapObject.parent) {
                currentTapObject.parent.remove(currentTapObject);
                console.log("Clone removido por triple click");
              }
            }
            currentTapObject.userData.cloneTapCount = 0;
          }, 250);
        } else if (currentTapObject.userData.rotatable) {
          tapCount++;
          if (tapCount === 1) {
            tapTimer = setTimeout(() => {
